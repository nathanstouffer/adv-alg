...
def outx(val):
    return str(round(val+trans_x, 3))
...
# trans_x is a float that will be added to all positions
script, file_name, trans_x = argv                                    # read in command line arguments
trans_x = float(trans_x)

tikz_scale   = 2                                                # scale value for the tikz picture
radius       = 0.025                                             # radius
...
out_file_name = file_name.split("/")[-1][:-3] + ".out"      # create output file name
